...
int main()
{
	...
	cout << "Будет проведено вычитание из наибольшего числа наименьшего" << endl;
	...
	LOOP_LBL_WRITE: mov al, a[ecx - 1];
		...
		add al, 10;
		mov remainder, 1;
		jmp NOT_MORE_THAN_10;
	NO_OVERFLOW_2:mov remainder, 0;
	NOT_MORE_THAN_10: xor edx, edx;
		mov ebx, 10;			//Получение последней цифры
		div ebx;
		mov esi, _offset;
		mov[result + esi], dl;	//Запись результата в массив
		add[result + esi], 48;	//Получение кода аски
		inc _offset;			//Увеличение смещения
		loop LOOP_LBL_WRITE;
