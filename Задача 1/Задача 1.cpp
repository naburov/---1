...
int main()
{
	...
	outer_lbl: mov ax, word ptr[N];			// Записываем счетчик в ax для вычисления смещения
		...
		mov minIndex, 0;					// Обнуление индекса столбца минимального элемента
	...
	get_column_index: push eax;		// Метод получения индекса столбца по индексу в одномерном массиве
		push ecx;
		push edx;
